...
const AddEventModal = ({ show, onHide, onSave, selectedDate, employees, initialNoAppointment = false }) => {
    const initialFormData = useMemo(() => ({
        ...
        technician4_id: '',
        equipment: '',
        no_appointment: initialNoAppointment
    }), [selectedDate, initialNoAppointment]);
    ...
    const handleFilePreview = (e, file) => {
        ...
        try {
            ...
            console.log(`👁️ [AddEventModal] Tentative de prévisualisation du fichier: ${file.name} (index ${fileIndex}/${installationFiles.length-1})`);
    ...
    const fetchInstallationFiles = async (installationNumber) => {
        if (!installationNumber) {
            console.warn('🚫 [AddEventModal] fetchInstallationFiles: Numéro d\'installation manquant');
            return;
        }
        
        try {
            setLoadingFiles(true);
            console.log('🔍 [AddEventModal] Chargement des fichiers pour l\'installation:', installationNumber);
            
            // Vérifier si l'utilisateur a des identifiants valides pour ProgressionLive
            if (!hasValidProgressionCredentials()) {
                console.log('🔑 [AddEventModal] Aucun identifiant valide pour ProgressionLive trouvé');
            ...
            if (result && result.success) {
                ...
                if (Array.isArray(result.data)) {
                    console.log(`📋 [AddEventModal] ${result.data.length} fichiers récupérés:`, result.data);
                    ...
                    console.warn('⚠️ [AddEventModal] Les données reçues ne sont pas un tableau:', result.data);
                    setInstallationFiles([]);
                }
            } else {
                // Vérifier si une connexion à ProgressionLive est nécessaire
                if (result && result.error === 'login_required') {
                    console.log('🔑 [AddEventModal] Connexion à ProgressionLive requise');
                    setShowLoginForm(true);
                } else {
                    console.error('❌ [AddEventModal] Erreur lors de la récupération des fichiers:', result?.error);
                }
                setInstallationFiles([]);
            }
        } catch (error) {
            console.error('💥 [AddEventModal] Exception lors de la récupération des fichiers:', error);
            setInstallationFiles([]);
        } finally {
    ...
    const handleLoginSuccess = (loginData) => {
        console.log('✅ [AddEventModal] Connexion à ProgressionLive réussie', loginData);
        ...
        setTimeout(() => {
            ...
            if (formData.installation_number) {
                console.log('🔄 [AddEventModal] Récupération des fichiers après connexion réussie...');
